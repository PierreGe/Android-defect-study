...
def main():
    ...
    #finaltarget = [('AdAway','org.adaway')]

    for appliName, signature in finaltarget:
        #cgscore = callgraphanalysis.score("data/" + appliName.replace(" ", "_") +"/callGraph/callgraph.txt", signature)
        ...
        for key in issuescore:
            if key in cgscore:
                for k in range(int(issuescore[key])):
                    x1.append(j)
                for k in range(int(cgscore[key])):
                    x2.append(j)
                j+=1
                xp1.append(issuescore[key])
                xp2.append(cgscore[key])


        if j>3:
            plt.hist(x1, bins=j, histtype='stepfilled', color='r', label='Issue')
            plt.hist(x2, bins=j, histtype='stepfilled', color='b', alpha=0.5, label='Calls')
            titleplt = appliName + str(max(pearsonr(xp1,xp2)))
            plt.title(titleplt)
            plt.xlabel("Class")
            plt.ylabel("Value")
            plt.legend()
            subfolder = "graphresultpullincludedclasscorrected"
